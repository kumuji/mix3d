...
from mix3d.models.model import HighDimensionalModel
from mix3d.models.wrapper import Wrapper
from mix3d.models.modules.common import convert_region_type
  ...
  """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Setup metadata
        super().__init__(nchannels, nchannels, config, D=D)
        ...
        if torch.prod(convert_to_int_tensor(model.OUT_PIXEL_DIST, model.D)) != 1:
            ...
            self.in_mapping = torch.sparse.FloatTensor(
                torch.stack(
                    (
                        in_cols.long(),
                        torch.arange(in_cols.size(0), out=torch.LongTensor()),
                    )
                ),
            ...
            self.out_mapping = torch.sparse.FloatTensor(
                torch.stack(
                    (
                        torch.arange(out_cols.size(0), out=torch.LongTensor()),
                        out_cols.long(),
                    )
                ),
            ...
            if self.config.is_cuda:
                self.in_mapping, self.out_mapping = (
                    self.in_mapping.cuda(),
                    self.out_mapping.cuda(),
                )
