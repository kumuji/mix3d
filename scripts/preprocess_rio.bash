...
function preprocess() {
    poetry run \
        python mix3d/datasets/preprocessing/rio_preprocessing.py preprocess_sequential \
        --data_dir="$DATA_DIR" \
        --save_dir="$SAVE_DIR" \
        --git_repo="$GIT_REPO" \
        --label_db="$LABEL_DB"
...
function make_instance_database() {
    poetry run \
        python mix3d/datasets/preprocessing/rio_preprocessing.py make_instance_database_sequential \
        --data_dir="$DATA_DIR" \
        --save_dir="$SAVE_DIR" \
        --label_db="$LABEL_DB"
}
...
function preprocess_dvc () {
    python -m dvc run \
        -d $LABEL_DB \
        -d mix3d/datasets/preprocessing/rio_preprocessing.py \
        -o "${SAVE_DIR}/train" \
        ...
        \
        poetry run \
            python mix3d/datasets/preprocessing/rio_preprocessing.py preprocess \
            --data_dir="$DATA_DIR" \
            --save_dir="$SAVE_DIR" \
            --git_repo="$GIT_REPO" \
            --label_db="$LABEL_DB"
...
function make_instance_database_dvc () {
    python -m dvc run \
        ...
        \
        poetry run \
            python mix3d/datasets/preprocessing/rio_preprocessing.py make_instance_database \
